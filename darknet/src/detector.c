...
{
    int use_shm = find_arg(argc, argv, "-use_shm"); // imsoo add
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box);
        ...
        // imsoo add
        if (use_shm) {
          demo_shm(cfg, weights, thresh, hier_thresh, cam_index, filename, names, classes, frame_skip, prefix, out_filename,
